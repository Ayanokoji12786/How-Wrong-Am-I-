...
function Metric({ label, value, detail, accent = false, index = 0 }: { label: string; value: ReactNode; detail: string; accent?: boolean; index?: number }) {
...
function App() {
  ...
  if (landing) return <><main className="landing"><nav className="landing-nav theme-light"><span className="brand"><i /> HOW WRONG AM I?</span><div><button className="text-button" onClick={() => setAuthOpen(true)}>Create account</button><button className="text-button" onClick={() => leaveLanding()}>Explore demo</button><button className="nav-cta" onClick={() => { leaveLanding(); setNewForecast(true) }}>Make a forecast</button></div></nav><section className="hero theme-light" ref={heroRef}><div className="hero-copy"><p className="eyebrow">A PERSONAL CALIBRATION JOURNAL</p><h1>Confidence is easy.<br /><em>Calibration</em> is measurable.</h1><p>Make predictions, define how they resolve, and discover whether your certainty meets reality.</p><div className="hero-actions"><button className="primary-button" onClick={() => { leaveLanding(); setNewForecast(true) }}>Test your calibration <span>→</span></button><button className="secondary-button" onClick={() => leaveLanding()}>See the evidence</button></div><small>Built for curiosity, not certainty theater.</small></div><TiltCard className="hero-chart"><div className="chart-heading"><span>DEMO CALIBRATION CURVE</span><b>{resolved.length} resolved forecasts</b></div><WebGLBoundary fallback={<CalibrationChart buckets={buckets} />}><Suspense fallback={<CalibrationChart buckets={buckets} />}><CalibrationScene3D buckets={buckets} /></Suspense></WebGLBoundary></TiltCard></section><section className="landing-loop"><motion.div className="landing-loop-line" initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true }} transition={{ duration: 1.1, ease: [0.16, 1, 0.3, 1] }} />{[['01', 'Forecast'], ['02', 'Define evidence'], ['03', 'Wait for reality'], ['04', 'Learn']].flatMap(([number, label], index) => [
  ...
  return <main className="app-shell">
    ...
    <section className="content"><header className="app-header"><div><span className="eyebrow">{accountName ? 'PERSONAL FORECASTER' : 'DEMO FORECASTER'}</span><h1>{view === 'dashboard' ? 'Your forecasting dashboard' : view === 'journal' ? 'Forecast journal' : 'What reality says'}</h1></div><button className="primary-button header-create" onClick={() => setNewForecast(true)}>+ New forecast</button></header>
      ...
      <section className="metrics-strip"><Metric label="CALIBRATION" value={score === null ? '—' : <AnimatedNumber value={score} />} detail={sampleLabel(resolved.length)} accent index={0} /><Metric label="BRIER" value={brier === null ? '—' : <AnimatedNumber value={brier} format={(n) => n.toFixed(3)} />} detail="Lower is better" index={1} /><Metric label="RESOLVED" value={<AnimatedNumber value={resolved.length} />} detail="Evidence collected" index={2} /><Metric label="ERROR" value={ece === null ? '—' : <AnimatedNumber value={ece * 100} format={(n) => `${n.toFixed(1)}%`} />} detail="Probability vs reality" index={3} /></section>
